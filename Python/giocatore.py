...
class Player():
    def __init__(self, x, y, selected, width, height, char_image):
        ...
        self.Name_animationBow = 'Characters'+self.Player_selected+"/Bow"
        ...
        self.animationBow = char_image[3]
        self.current_spriteBow = 0
...
# ---------- self.set() ----------
    ...
    def setBow(self, s):
        if not s:
            self.current_spriteBow = 0
        self.__bow_pressed = s
        
    def getBow(self):
        return self.__bow_pressed
...
# ---------- self.get() ----------
    ...
    def character_update(self,var):
        ...
        if l and not r:
            ...
            self.current_spriteBow += 0.3 / GLOB.Delta_Time

            if self.current_spriteBow >= len(self.animationBow):
                self.setBow(False)
            
            self.image = self.animationBow[int(self.current_spriteBow)]
        ...
        if var==4:
            self.character = pygame.image.load(
            os.path.join(self.Name_animationBow,self.image)).convert_alpha()           
