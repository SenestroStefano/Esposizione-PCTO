import pandas as pd
import pygame, os, sys, random, re
...
def SetPlayer_sprite():
    global Folder_walkO, Folder_walkVD, Folder_walkVU, Folder_bow
    ...
    Folder_bow = 'Characters'+GLOB.scelta_rep+'/Bow'
    ...
    def riempi(percorso):
        ...
        for filename in FileNames:
            ...
            if percorso == Folder_bow:
                GLOB.PlayerBow.append(filename)
...
def inizializza():
    ...
    character_image = (GLOB.PlayerWalkingVD,GLOB.PlayerWalkingVU,GLOB.PlayerWalkingO, GLOB.PlayerBow)
...
def main():
    ...
    def key_pressed(event, IsPressed):
        ...
        if LEFT and not RIGHT and not(condition_1 and condition_2):
            ...
            if not player.getBow() and not GLOB.PlayerIsMoving:
                player.setBow(True)
